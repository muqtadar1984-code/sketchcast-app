...
- **Disposable addresses are refused** (migration `0117`). A `BEFORE INSERT OR UPDATE OF email` trigger on `auth.users` raises when the address's domain — or any parent domain — is in `public.disposable_email_domains` (service-role only; seeded from the community `disposable-email-domains` list, ~8.9k rows, and extendable with a plain `INSERT` from the SQL editor, no deploy). Because the browser calls `signUp` directly, a trigger refusal reaches it only as GoTrue's "Database error saving new user", so `/signup`, `/schoolsignup` and `/invite/[token]` first call the same rule — `email_domain_is_disposable(text)`, granted to `anon` — via `src/utils/disposable-email.ts` and show `app.auth.disposableEmail` instead. The pre-check fails open (an RPC error proceeds to `signUp`); the trigger is the only gate. Existing accounts on a listed domain are untouched; provisioned students (`@students.sketchcast.app`) and Google addresses are never on the list. Verification script: `supabase/tests/0117_disposable_email_test.sql`.
...
- **Already-generated kinds** render their `ContentCell` (`content-cell.tsx`) instead: status pill (`queued`/`{progress}%`), or `done` download links, or a `failed` state with retry + report. For a `presentation` this means **▶ Watch** (opens the in-app player in `lesson-player.tsx` — never the raw signed URL in a new tab; every `<video>` on the site sets `controlsList="nodownload"`, refuses the context menu and disables picture-in-picture/remote playback, so the browser offers no save route) + **⬇ Deck**, plus an **Ask Coach** entry point and Regenerate/Delete/Report; for document kinds a **⬇ Download**.
...
Lives in the **landing repo** (static, Cloudflare) at `sketchcast.app/pricing`. `pricing.config.js` is the single source of truth: `pricing.html` renders entirely from `window.SKETCHCAST_PRICING`, so no price or link is written twice. Because it's a static site with no runtime env and the LS URLs are **public hosted-checkout links** anyway, the checkout URLs live directly in `pricing.config.js` (no secrets in this repo). An LS link is **variant-locked** — `/checkout/buy/<slug>` opens one variant at one price with no billing-cycle chooser — so each paid plan carries `checkout: { monthly, annual }` and the Monthly/Annual toggle re-points every CTA `href` in the same update that re-renders its price. (This paragraph previously said one URL served both cycles; it never did, and a visitor who toggled to Annual and clicked was billed monthly.) Copy: teacher plans $24/$49 monthly (annual = ×10), Family $9.99/$99. The **Founding** offer is **code-driven, not a special link** — the CTA opens the Teacher Pro **monthly** checkout (the variant the $14-off code is priced against) and the page shows the copyable `FOUNDINGTEACHER` code. **Schools** never see a public price — CTA is a sales-enquiry mailto only.
